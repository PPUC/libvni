...
#define FRAMEUTIL_VERSION                                       \
  ...
  "." FRAMEUTIL_STR(FRAMEUTIL_VERSION_MINOR) "." FRAMEUTIL_STR( \
      FRAMEUTIL_VERSION_PATCH)
#define FRAMEUTIL_MINOR_VERSION          \
  FRAMEUTIL_STR(FRAMEUTIL_VERSION_MAJOR) \
  "." FRAMEUTIL_STR(FRAMEUTIL_VERSION_MINOR)
...
enum class ColorMatrix { Rgb, Rbg };

class Helper {
 public:
  static int MapAdafruitIndex(int x, int y, int width, int height,
                              int numLogicalRows);
  static void ConvertToRgb24(uint8_t* pFrameRgb24, uint8_t* pFrame, int size,
                             uint8_t* pPalette);
  static void Split(uint8_t* pPlanes, uint16_t width, uint16_t height,
                    uint8_t bitlen, uint8_t* pFrame);
  static void SplitIntoRgbPlanes(const uint16_t* rgb565, int rgb565Size,
                                 int width, int numLogicalRows, uint8_t* dest,
                                 ColorMatrix colorMatrix = ColorMatrix::Rgb);
  static float CalcBrightness(float x);
  static void ScaleDownIndexed(uint8_t* pDestFrame, const uint16_t destWidth,
                               const uint8_t destHeight,
                               const uint8_t* pSrcFrame,
                               const uint16_t srcWidth,
                               const uint8_t srcHeight);
  static void ScaleDownPUP(uint8_t* pDestFrame, const uint16_t destWidth,
                           const uint8_t destHeight, const uint8_t* pSrcFrame,
                           const uint16_t srcWidth, const uint8_t srcHeight);
  static void ScaleDown(uint8_t* pDestFrame, const uint16_t destWidth,
                        const uint8_t destHeight, const uint8_t* pSrcFrame,
                        const uint16_t srcWidth, const uint8_t srcHeight,
                        uint8_t bits);
  static void ScaleUpIndexed(uint8_t* pDestFrame, const uint8_t* pSrcFrame,
                             const uint16_t srcWidth, const uint8_t srcHeight);
  static void ScaleUp(uint8_t* pDestFrame, const uint8_t* pSrcFrame,
                      const uint16_t srcWidth, const uint8_t srcHeight,
                      uint8_t bits);
  static void CenterIndexed(uint8_t* pDestFrame, const uint16_t destWidth,
                            const uint8_t destHeight, const uint8_t* pSrcFrame,
                            const uint16_t srcWidth, const uint8_t srcHeight);
  static void Join(uint8_t* pFrame, uint16_t width, uint16_t height,
                   uint8_t bitlen, const uint8_t* pPlanes);
  ...
  static void CombinePlaneWithMask(const uint8_t* planeA, const uint8_t* planeB,
                                   const uint8_t* mask, uint8_t* out,
                                   size_t len);
  static uint8_t ReverseByte(uint8_t value);
  static uint32_t Checksum(const uint8_t* input, size_t len,
                           bool reverse = false);
  static uint32_t ChecksumWithMask(const uint8_t* input, const uint8_t* mask,
                                   size_t len, bool reverse = false);
  static void Scale2XIndexed(uint8_t* pDestFrame, const uint8_t* pSrcFrame,
                             uint16_t srcWidth, uint16_t srcHeight);
  static void ScaleDoubleIndexed(uint8_t* pDestFrame, const uint8_t* pSrcFrame,
                                 uint16_t srcWidth, uint16_t srcHeight);

  static void Center(uint8_t* pDestFrame, const uint16_t destWidth,
                     const uint8_t destHeight, const uint8_t* pSrcFrame,
                     const uint16_t srcWidth, const uint8_t srcHeight,
                     uint8_t bits);
};

inline int Helper::MapAdafruitIndex(int x, int y, int width, int height,
                                    int numLogicalRows) {
...
inline void Helper::Split(uint8_t* pPlanes, uint16_t width, uint16_t height,
                          uint8_t bitlen, uint8_t* pFrame) {
...
inline void Helper::ConvertToRgb24(uint8_t* pFrameRgb24, uint8_t* pFrame,
                                   int size, uint8_t* pPalette) {
  for (int i = 0; i < size; i++) {
    memcpy(&pFrameRgb24[i * 3], &pPalette[pFrame[i] * 3], 3);
  }
}

inline void Helper::SplitIntoRgbPlanes(const uint16_t* rgb565, int rgb565Size,
                                       int width, int numLogicalRows,
...
inline void Helper::ScaleDownIndexed(uint8_t* pDestFrame,
  ...
  // for half scaling we take the 4 points and look if there is one color
  // repeated
  for (uint8_t y = 0; y < srcHeight; y += 2) {
    ...
    for (uint16_t x = 0; x < srcWidth; x += 2) {
      ...
      if (x < srcWidth / 2) {
        if (y < srcHeight / 2) {
          if (upper_left == upper_right || upper_left == lower_left ||
          ...
          if (lower_left == lower_right || lower_left == upper_left ||
        ...
        if (y < srcHeight / 2) {
          if (upper_right == upper_left || upper_right == lower_right ||
          ...
          if (lower_right == lower_left || lower_right == upper_right ||
    ...
    memcpy(&pDestFrame[(yOffset + (y / 2)) * destWidth + xOffset], row.data(),
           srcWidth / 2);
  }
}

inline void Helper::ScaleDownPUP(uint8_t* pDestFrame, const uint16_t destWidth,
                                 const uint8_t destHeight,
                                 const uint8_t* pSrcFrame,
                                 const uint16_t srcWidth,
                                 const uint8_t srcHeight) {
...
inline void Helper::ScaleDown(uint8_t* pDestFrame, const uint16_t destWidth,
                              const uint8_t destHeight,
                              const uint8_t* pSrcFrame, const uint16_t srcWidth,
                              const uint8_t srcHeight, uint8_t bits) {
  uint8_t xOffset = (destWidth - (srcWidth / 2)) / 2;
  uint8_t yOffset = (destHeight - (srcHeight / 2)) / 2;
  uint8_t bytes =
      bits / 8;  // RGB24 (3 byte) or RGB16 (2 byte) or indexed (1 byte)
  ...
  for (uint8_t y = 0; y < srcHeight; y += 2) {
    ...
    for (uint16_t x = 0; x < srcWidth; x += 2) {
      if (x < srcWidth / 2) {
        if (y < srcHeight / 2) {
          if (memcmp(&pSrcFrame[upper_left], &pSrcFrame[upper_right], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[upper_left], &pSrcFrame[lower_left], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[upper_left], &pSrcFrame[lower_right], bytes) ==
                  0)
            memcpy(&pDestFrame[target], &pSrcFrame[upper_left], bytes);
          else if (memcmp(&pSrcFrame[upper_right], &pSrcFrame[lower_left],
                          bytes) == 0 ||
                   memcmp(&pSrcFrame[upper_right], &pSrcFrame[lower_right],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[upper_right], bytes);
          else if (memcmp(&pSrcFrame[lower_left], &pSrcFrame[lower_right],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[lower_left], bytes);
          else
            memcpy(&pDestFrame[target], &pSrcFrame[upper_left], bytes);
        } else {
          if (memcmp(&pSrcFrame[lower_left], &pSrcFrame[lower_right], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[lower_left], &pSrcFrame[upper_left], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[lower_left], &pSrcFrame[upper_right], bytes) ==
                  0)
            memcpy(&pDestFrame[target], &pSrcFrame[lower_left], bytes);
          else if (memcmp(&pSrcFrame[lower_right], &pSrcFrame[upper_left],
                          bytes) == 0 ||
                   memcmp(&pSrcFrame[lower_right], &pSrcFrame[upper_right],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[lower_right], bytes);
          else if (memcmp(&pSrcFrame[upper_left], &pSrcFrame[upper_right],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[upper_left], bytes);
          else
            memcpy(&pDestFrame[target], &pSrcFrame[lower_left], bytes);
        ...
        if (y < srcHeight / 2) {
          if (memcmp(&pSrcFrame[upper_right], &pSrcFrame[upper_left], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[upper_right], &pSrcFrame[lower_right], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[upper_right], &pSrcFrame[lower_left], bytes) ==
                  0)
            memcpy(&pDestFrame[target], &pSrcFrame[upper_right], bytes);
          else if (memcmp(&pSrcFrame[upper_left], &pSrcFrame[lower_right],
                          bytes) == 0 ||
                   memcmp(&pSrcFrame[upper_left], &pSrcFrame[lower_left],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[upper_left], bytes);
          else if (memcmp(&pSrcFrame[lower_right], &pSrcFrame[lower_left],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[lower_right], bytes);
          else
            memcpy(&pDestFrame[target], &pSrcFrame[upper_right], bytes);
        } else {
          if (memcmp(&pSrcFrame[lower_right], &pSrcFrame[lower_left], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[lower_right], &pSrcFrame[upper_right], bytes) ==
                  0 ||
              memcmp(&pSrcFrame[lower_right], &pSrcFrame[upper_left], bytes) ==
                  0)
            memcpy(&pDestFrame[target], &pSrcFrame[lower_right], bytes);
          else if (memcmp(&pSrcFrame[lower_left], &pSrcFrame[upper_right],
                          bytes) == 0 ||
                   memcmp(&pSrcFrame[lower_left], &pSrcFrame[upper_left],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[lower_left], bytes);
          else if (memcmp(&pSrcFrame[upper_right], &pSrcFrame[upper_left],
                          bytes) == 0)
            memcpy(&pDestFrame[target], &pSrcFrame[upper_right], bytes);
          else
            memcpy(&pDestFrame[target], &pSrcFrame[lower_right], bytes);
...
inline void Helper::ScaleUp(uint8_t* pDestFrame, const uint8_t* pSrcFrame,
                            const uint16_t srcWidth, const uint8_t srcHeight,
                            uint8_t bits) {
  uint8_t bytes =
      bits / 8;  // RGB24 (3 byte) or RGB16 (2 byte) or indexed (1 byte)
    ...
    for (uint16_t x = 0; x < srcWidth; x++) {
      for (uint8_t tc = 0; tc < bytes; tc++) {
        if (x == 0 && y == 0) {
          ...
          e[tc] = f[tc] = h[tc] = i[tc] =
              pSrcFrame[srcHeight * row - bytes + tc];
        } else if (x == 0) {
          a[tc] = b[tc] = pSrcFrame[(y - 1) * row + tc];
          c[tc] = pSrcFrame[(y - 1) * row + bytes + tc];
      ...
      if (memcmp(b, h, bytes) != 0 && memcmp(d, f, bytes) != 0) {
        memcpy(&pDestFrame[(y * 2 * destWidth + x * 2) * bytes],
               memcmp(d, b, bytes) == 0 ? d : e, bytes);
        memcpy(&pDestFrame[(y * 2 * destWidth + x * 2 + 1) * bytes],
               memcmp(b, f, bytes) == 0 ? f : e, bytes);
        memcpy(&pDestFrame[((y * 2 + 1) * destWidth + x * 2) * bytes],
               memcmp(d, h, bytes) == 0 ? d : e, bytes);
        memcpy(&pDestFrame[((y * 2 + 1) * destWidth + x * 2 + 1) * bytes],
               memcmp(h, f, bytes) == 0 ? f : e, bytes);
      } else {
        memcpy(&pDestFrame[(y * 2 * destWidth + x * 2) * bytes], e, bytes);
        memcpy(&pDestFrame[(y * 2 * destWidth + x * 2 + 1) * bytes], e, bytes);
        memcpy(&pDestFrame[((y * 2 + 1) * destWidth + x * 2) * bytes], e,
               bytes);
        memcpy(&pDestFrame[((y * 2 + 1) * destWidth + x * 2 + 1) * bytes], e,
               bytes);
      }
    }
  }
...
inline void Helper::ScaleUpIndexed(uint8_t* pDestFrame,
                                   const uint8_t* pSrcFrame,
                                   const uint16_t srcWidth,
                                   const uint8_t srcHeight) {
...
inline void Helper::Center(uint8_t* pDestFrame, const uint16_t destWidth,
                           const uint8_t destHeight, const uint8_t* pSrcFrame,
                           const uint16_t srcWidth, const uint8_t srcHeight,
                           uint8_t bits) {
  ...
  for (uint8_t y = 0; y < srcHeight; y++) {
    memcpy(&pDestFrame[((yOffset + y) * destWidth + xOffset) * bytes],
           &pSrcFrame[y * srcWidth * bytes], srcWidth * bytes);
  }
}

inline void Helper::CenterIndexed(uint8_t* pDestFrame, const uint16_t destWidth,
                                  const uint8_t destHeight,
                                  const uint8_t* pSrcFrame,
                                  const uint16_t srcWidth,
                                  const uint8_t srcHeight) {
...
inline uint32_t Helper::Checksum(const uint8_t* input, size_t len,
                                 bool reverse) {
...
inline uint32_t Helper::ChecksumWithMask(const uint8_t* input,
  ...
  if (!reverse) {
    ...
    for (size_t i = 0; i < len; i++) {
      cs = (cs >> 8) ^
           kChecksumTable[(cs ^ (kReverseByte[input[i]] & mask[i])) & 0xFFu];
    }
  }
  return cs ^ 0xFFFFFFFFu;
}

inline void Helper::Join(uint8_t* pFrame, uint16_t width, uint16_t height,
                         uint8_t bitlen, const uint8_t* pPlanes) {
  int planeSize = width * height / 8;
...
inline void Helper::CombinePlaneWithMask(const uint8_t* planeA,
                                         const uint8_t* planeB,
                                         const uint8_t* mask, uint8_t* out,
                                         size_t len) {
  for (size_t i = 0; i < len; i++) {
    out[i] =
        static_cast<uint8_t>((planeA[i] & mask[i]) | (planeB[i] & ~mask[i]));
  }
}

inline void Helper::ScaleDoubleIndexed(uint8_t* pDestFrame,
                                       const uint8_t* pSrcFrame,
                                       uint16_t srcWidth, uint16_t srcHeight) {
  uint16_t destWidth = srcWidth * 2;
...
inline void Helper::Scale2XIndexed(uint8_t* pDestFrame,
                                   const uint8_t* pSrcFrame, uint16_t srcWidth,
                                   uint16_t srcHeight) {
